...
def connect_regions(world: WildfrostWorld) -> None:
    ...
    if (world.options.fight_gating.value == 1 or world.options.fight_gating.value == 3):
        ...
        snowdwell.connect(shademancers, "Unlock Shademancers and Fight 2", lambda state: state.has("Shademancers Tribe", world.player) and state.count("Progressive Fight", world.player) > 0)
        snowdwell.connect(clunkmasters, "Unlock Clunkmasters and Fight 2", lambda state: state.has("Clunkmasters Tribe", world.player) and state.count("Progressive Fight", world.player) > 0)
    else:
